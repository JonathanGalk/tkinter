...
class Main:
    def __init__(self, master):
        ...
        self.menuControle.add_command(label='Salvar Mp3', command=self.salvar2)
        ...
        self.menu.add_cascade(label='Ajuda', command=self.ajuda)
    ...
    def salvar(self):
        ...
        filename.close()
        root.title('Bloco - ' + filename3[1].split('/')[-1].replace("'", ''))
    ...
    def salvar2(self):
        ...
        filename3 = filename2.split("'")
        print(filename3)
        fn = filename3[0].replace(' ', '_')
        voz.save(fn)      
        root.title('Bloco - ' + filename3[0].split('/')[-1].replace("'", ''))
        messagebox.showinfo('Salvar Mp3', 'Salvo com sucesso')
    ...
    def abrir(self):
        ...
        filename3 = filename2.split('/')
        f.close()
        root.title('Bloco - ' + filename3[-1].replace("'", ''))
        
    def sobre(self):
        messagebox.showinfo('Sobre',
        '''Bloco de que salva como texto e audio(mp3),
bem simples feito em python para estudos.
        Autor: Jonathan
        Versão: 1.0
        Versão Python: 3.7''')
...
'''Quando escolher salvar em mp3, espere o aviso de que o arquivo foi salvo
com sucesso, caso contraio, poderá haver erros.''')
