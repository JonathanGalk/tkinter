...
# ---------------------- CONEXÃO COM BANCO DE DADOS ----------------
        ...
        self.c.execute('''
        CREATE TABLE IF NOT EXISTS clientes(id integer primary key not null,
    ...
    def cadastrar(self):
        ...
        if nome == '':
            ...
            #insere os dados na tebela e em seguida apaga os campos
                ...
                messagebox.showinfo('Aviso','Os campos RG,CPF,Nº Casa,Telefone e Celular usam somente números.')
    ...
    def pes_id(self):
        ...
        if ids == '':
            ...
            #verica se um numero foi digitado e logo em seguida cria uma lista
            #com todos os ID's validos e verifica se esse ID existe
            if str(ids).isdigit():
                for id in self.c.execute(sql2):
                    lista.append(id[0])
                if int(ids) in lista:
                    ...
                    messagebox.showinfo('Aviso','ID não existe!')
            else:
                messagebox.showinfo('Aviso','Escolha um ID válido(número)')
